...
def initialize_cache(inputs, outputs, targets, max_decode_len, config, lp_config):
...
def main(_):
  ...
  lp_train_config = base_config.replace(vocab_size=io_vocab_size,
                                        output_vocab_size=FLATS.latent_vocab_size)
  ...
  for step in range(start_step, FLAGS.num_train_steps):
    ...
    state, metrics, latent_metrics, dropout_rng = p_train_step(
    ...
    if (step and step % FLAGS.eval_freq == 0) or is_last_step:
      ...
      for batches in eval_ds.as_numpy_iterator():
        ...
        metrics, latent_metrics = p_eval_step(state, inputs, outputs, targets)
    ...
    if (step and step % FLAGS.predict_freq == 0) or is_last_step:
      ...
      for dataset, predict_or_test in [(predict_ds, 'predict'),
        ...
        for beam_size in [1, 10]:
          ...
          ios, targets_list, predictions, latent_predictions, top_of_beams, scores = (
              [], [], [], [], [], [])
          for batches in dataset.as_numpy_iterator():
            ...
            cache, lp_cache = (p_init_cache(inputs, outputs, targets)
                     if not FLAGS.slow_decode else None)
            predicted, latent_predicted = p_pred_step(state, inputs, outputs, cache, lp_cache,
                                                      beam_size)
